 ...
 */
public class MsWordXFormatConverter extends AbstractStringBasedConverter {

    /** The MIME type supported by this DocumentFilter */
    public static final String MS_WORD_X_MIMETYPE = "application/vnd.openxmlformats-officedocument.wordprocessingml.document";
	...
	protected String getContentFromStream(InputStream input) throws FormatConverterException {
        try {
        	...
        	XWPFWordExtractor theExtr = new XWPFWordExtractor(thePack);
        	String theContents = theExtr.getText();
        	return theContents;
        } catch (Exception e) {
	...
	protected void fillMimeTypeMappings(Collection<ConverterMapping> collection) {
    	collection.add(new ConverterMapping(MS_WORD_X_MIMETYPE, AbstractFormatConverter.TXT_MIMETYPE));
    }
