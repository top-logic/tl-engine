...
/**
 * Captures a sequence of user interactions on one window as replayable {@link ReactCommand} items.
 *
 * <p>
 * While {@link #isRecording() recording}, the React command servlet feeds each dispatched command
 * (as its typed item, addressed by a semantic {@link ReactCommand#getAddress() address}) into
 * {@link #record(ReactCommand)}. The captured list is the successor to a legacy
 * {@code ScriptingRecorder} trace: a script that {@link ScriptingSession#act(String, String,
 * java.util.Map) replays} through the same command path the browser uses.
 * </p>
 *
 * <p>
 ...
 * after each {@link #start()}, {@link #stop()} and {@link #record(ReactCommand)}, so a view in another
...
public final class ScriptRecorder {
	...
	private final List<ReactCommand> _steps = new ArrayList<>();
	 ...
	 *        The step to capture.
	 */
	public void record(ReactCommand step) {
		record(step, false);
	}
	 ...
	 *        {@link ReactCommand#getAddress() address} with the same {@link ReactCommand#getName()
	 *        command name}, the previous step is <em>superseded</em> by this one rather than a new step
	 ...
	 */
	public void record(ReactCommand step, boolean coalescing) {
		boolean captured;
		synchronized (this) {
			captured = _recording;
			if (captured) {
	 ...
	 */
	private boolean supersedesLast(ReactCommand step) {
		if (_steps.isEmpty()) {
			return false;
		}
		ReactCommand last = _steps.get(_steps.size() - 1);
		return last.getAddress() != null
	 ...
	 */
	public synchronized List<ReactCommand> steps() {
