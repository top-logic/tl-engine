import { React as d, useTLState as h, useTLCommand as b, register as y } from "tl-react-bridge";
import { styleTags as Y, tags as O, LRParser as C, LRLanguage as Z, LanguageSupport as E, CodeEditor as D } from "tl-code-editor";
const X = Y({
  Number: O.number,
  String: O.string,
  Boolean: O.bool,
  ...
  '"?" ":"': O.punctuation
}), W = { __proto__: null, true: 40, false: 42, null: 46, tuple: 62, switch: 68, default: 74 }, x = C.deserialize({
  version: 14,
  ...
  tokenizers: [0, 1, 2, 3],
  topRules: { Script: [0, 3] },
  specialized: [{ term: 62, get: (n) => W[n] || -1 }],
  tokenPrec: 1011
}), $ = Z.define({
  parser: x.configure({
    props: [X]
...
const { useRef: q, useEffect: v, useCallback: p, useMemo: z } = d, G = 3e3, U = ({ controlId: n, state: i }) => {
  const l = h(), Q = b(), P = q(null), m = i.value ?? "", T = i.readOnly === !0, R = l.diagnostics ?? [], f = z(() => k(), []), V = p(
    (e) => {
      const a = e.pos, r = e.state.doc.lineAt(a);
      if (!e.matchBefore(/[\w$`.:]+/) && !e.explicit) return Promise.resolve(null);
      const t = e.matchBefore(/[\w]+/), g = (t == null ? void 0 : t.text) ?? "", w = (t == null ? void 0 : t.from) ?? a, j = r.text.substring(0, a - r.from), c = String(Date.now()) + Math.random();
      return Q("complete", { line: j, prefix: g, requestId: c }), new Promise((u) => {
        P.current = { requestId: c, resolve: u, from: w }, setTimeout(() => {
          var S;
          ((S = P.current) == null ? void 0 : S.requestId) === c && (P.current = null, u(null));
        }, G);
      });
    },
    [Q]
  ...
  v(() => {
    const e = P.current;
    if (!e || !o || o.requestId !== e.requestId)
      return;
    const a = o.completions ?? [];
    ...
    e.resolve({
      ...
      options: a.map((r) => ({
        label: r.name,
        apply: r.replacement ?? r.name,
        detail: r.value !== r.name ? r.value : void 0,
        info: r.docHTML ? () => {
          const s = document.createElement("div");
          return s.innerHTML = r.docHTML, s;
        } : void 0,
        boost: r.score ?? 0
      }))
    });
  ...
  const _ = p(
    (e) => {
      Q("valueChanged", { value: e }), Q("validate", { text: e });
    },
    [Q]
  );
