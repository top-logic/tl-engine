...
/**
 * {@link List} that automatically indexes its elements using a given
 * {@link Mapping}.
 * 
 * <p>
 * Elements with the same key can only appear once in this list, but the order
 * of elements is preserved.
 * </p>
 * 
 * @author <a href="mailto:bhu@top-logic.com">Bernhard Haumacher</a>
 */
public class IndexedList<K, V> extends ObservableList<V> {
	private final Map<K, V> index;
	private final Mapping<? super V, ? extends K> keyMapping;

	/**
	 * Creates a {@link IndexedList}.
	 * 
	 * @param indexMap
	 ...
	 *        collection.
	 */
	public IndexedList(Map<K, V> indexMap, Mapping<? super V, ? extends K> keyMapping) {
		this.index = indexMap;
	...
	protected void beforeAdd(V newElement) {
		...
		if (clash != null) {
			...
			throw new IllegalArgumentException("List already contains element with key '" + key + "'.");
		}
	}
