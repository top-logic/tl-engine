	...
	CKEDITOR.dialog.add( 'templates', function( editor ) {
		...
		function renderTemplatesList( container, templatesDefinitions ) {
			...
			for ( var i = 0, totalDefs = templatesDefinitions.length; i < totalDefs; i++ ) {
				...
				if(definition == null) {
					var baseDefinition = templatesDefinitions[ i ];
					var index = baseDefinition.indexOf("_");
					if(index > -1) {
						baseDefinition = baseDefinition.substring(index);
					}
					definition = CKEDITOR.getTemplates( CKEDITOR.config.fallback_language +  baseDefinition );
					if(definition == null) {
						var item = createNoTranslationItem( templatesDefinitions[ i ] );
						container.append( item );
						continue;
					}
				}
				
				var	imagesPath = definition.imagesPath,
		...
		function createNoTranslationItem (template) {
			var item = CKEDITOR.dom.element.createFromHtml( '<div class="cke_tpl_item"></div>');

			// Build the inner HTML of our new item DIV.
			var html = '<table style="width:350px;" class="cke_tpl_preview" role="presentation"><tr>';

			html += '<td style="white-space:normal;"><span class="cke_tpl_title" style="color: #acabab">' + template + '</span><br/>';

			html += '<span style="color: #acabab">' + editor.lang.templates.noTranslation + '</span>';
