...
| Detail drawer | **Migrated** | `demo/detail-drawer-demo.view.xml` (`<adaptive-detail detail-display="drawer">`, a full-width ticket table whose detail overlays it from the right edge) over the `tl.demo.projectManagement` model + seed data. |
...
| Page form (`<form>` in the fill contract) | **Migrated** | `demo/form-fill-demo.view.xml` — a selector table beside a page-spanning `<form>` around a nested `<split-panel>`: the form follows the fill contract, so the inner panes are sized by the frame, and one Edit/Save/Cancel set in the panel toolbar edits the whole right side. |
