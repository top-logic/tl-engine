...
public interface ViewCommandConfirmation {
	 ...
	 *
	 * @param context
	 *        The {@link ReactContext}, e.g. to resolve additional input channels.
	 * @param commandLabel
	 *        The resolved label of the command (may be {@code null} if the command has no label).
	 * @param input
	 *        The current input value (may be {@code null}).
	 * @return The confirmation message to display, or {@code null} to skip confirmation (execute
	 *         immediately).
	 */
	ResKey getConfirmation(ReactContext context, ResKey commandLabel, Object input);
}
