...
public class AWTContext implements RenderContext {
	...
	public TextMetricsImpl measure(String text) {
		return measure(text, null, 0, null);
	}
	...
	/**
	 * The {@link Font} to measure with.
	 *
	 * @param fontSize
	 *        The requested size in CSS pixels, or {@code 0} for the context default.
	 */
	private Font font(String fontFamily, double fontSize, String fontWeight) {
		String family = (fontFamily != null && !fontFamily.isEmpty()) ? fontFamily : DEFAULT_FONT_FAMILY;
		float sizePt = fontSize > 0 ? (float) (fontSize * PT_PER_PX) : _defaultSizePt;
