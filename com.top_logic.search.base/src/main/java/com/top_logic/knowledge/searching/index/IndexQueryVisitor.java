...
/** 
 * A query visitor providing full text searches for index search machines.
 *
 * @author    <a href="mailto:dro@top-logic.com">Dieter Rothb&auml;cher</a>
 */
public class IndexQueryVisitor extends AbstractVisitor {
     ...
     */
    public IndexQueryVisitor() {
        this("AND", "OR", true);
    }

    public IndexQueryVisitor(String anAnd, String anOr, boolean withBrackets) {
        this.and      = anAnd;
        this.or       = anOr;
    ...
    /**
     * Visit the Query and return the resulting String.
     */
	...
	public void visitBinaryNode(BinaryNode aNode) throws QueryException {
        ...
        if (isOperation) {
            ...
            if ((theOp == QueryPrefixConstants.OR) && (this.or != null)) {
                this.result.append(this.or).append(' ');
            }
            else if ((theOp == QueryPrefixConstants.AND) && (this.and != null)) {
                this.result.append(this.and).append(' ');
            }
        }
