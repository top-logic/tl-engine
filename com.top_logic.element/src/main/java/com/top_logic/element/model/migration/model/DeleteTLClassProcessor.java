...
public class DeleteTLClassProcessor extends AbstractConfiguredInstance<DeleteTLClassProcessor.Config>
	...
	private void internalDoMigration(Log log, PooledConnection connection) throws SQLException, MigrationException {
		...
		Type type;
		try {
			type = Util.getTLTypeOrFail(connection, classToDelete);
		} catch (MigrationException ex) {
			log.info(
				"No class with name '" + Util.qualifiedName(classToDelete) + "' to delete available at "
					+ getConfig().location(),
				Log.WARN);
			return;
		}

		Util.deleteTLType(connection, type, getConfig().isFailOnExistingAttributes());
		log.info("Deleted type " + Util.toString(type) + ".");
		...
		if (getConfig().getTypeTable() != null) {
			...
			delete(
				...
				and(
					...
					eqSQL(
						column(Util.refID(PersistentObject.TYPE_REF)),
			...
			log.info("Deleted " + deletedRows + " instances of type '" + Util.toString(type) + "' from table "
