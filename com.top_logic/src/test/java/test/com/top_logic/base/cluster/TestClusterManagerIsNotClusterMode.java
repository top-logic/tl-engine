 ...
 * 
 * This test expects the manager is not in cluster mode.
 * 
 * @see TestClusterManagerIsClusterMode
 * 
 * @since 5.8.0
 * 
 * @author <a href="mailto:daniel.busche@top-logic.com">Daniel Busche</a>
 */
public class TestClusterManagerIsNotClusterMode extends BasicTestCase {

	/**
	 * Tests the static method {@link ClusterManager#isClusterModeConfigured()}.
	 */
	public void testStatic() {
		assertFalse(ClusterManager.isClusterModeConfigured());
	}
	...
	public void testInstance() {
		ModuleUtil.INSTANCE.inModuleContext(ClusterManager.Module.INSTANCE, new Computation<Void>() {
			...
			@Override
			public Void run() {
				assertFalse(ClusterManager.getInstance().isClusterMode());
				return null;
			}
		});
	}
	...
	/**
	 * a cumulative {@link Test} for all Tests in {@link TestClusterManagerIsNotClusterMode}.
	 */
	public static Test suite() {
		Test innerTest = new TestSuite(TestClusterManagerIsNotClusterMode.class);
		innerTest = ClusterManagerSetup.setupClusterTables(innerTest);
		innerTest = ServiceTestSetup.startConnectionPool(innerTest);
		String customFile = CustomPropertiesDecorator.createFileName(TestClusterManagerIsNotClusterMode.class);
