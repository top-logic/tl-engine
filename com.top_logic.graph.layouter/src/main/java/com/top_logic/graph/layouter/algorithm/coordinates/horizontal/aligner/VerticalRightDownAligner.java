...
/**
 * A {@link VerticalDownAligner} which aligns from right to left.
 *
...
public class VerticalRightDownAligner extends VerticalDownAligner {
	...
	/**
	 * Creates a {@link VerticalRightDownAligner}.
	 */
	public VerticalRightDownAligner(LayoutGraph graph, LayoutDirection direction) {
		...
		for (int i = 2; i <= amountOfLayers; i++) {
			...
			for (int k = nodesSize - 1; k >= 0; k--) {
				...
				if (!currentNode.isTargetDummy()) {
					...
					List<Integer> medianIndices = getMedianIndices(upperNodes, true);
					...
					List<LayoutNode> topNodes =
						new LinkedList<>(LayoutGraphUtil.getTopNodes(getDirection(), currentNode));
