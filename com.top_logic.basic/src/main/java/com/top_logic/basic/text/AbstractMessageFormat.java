 ...
 */
@FrameworkInternal
public class AbstractMessageFormat extends Format {
     ...
     */
    public AbstractMessageFormat(String pattern) {
     ...
     * @since 1.4
     */
    public AbstractMessageFormat(String pattern, Locale locale) {
        this.locale = locale;
        applyPattern(pattern);
    }
        ...
        for (int i = 0; i <= maxOffset; ++i) {
            ...
            if (fmt == null) {
                ...
                for (index = MODIFIER_DEFAULT; index < DATE_TIME_MODIFIERS.length; index++) {
					DateFormat df = createDateFormat(DATE_TIME_MODIFIERS[index]);
                    if (fmt.equals(df)) {
                        result.append(",date");
                        break;
                    }
					df = createTimeFormat(DATE_TIME_MODIFIERS[index]);
                    if (fmt.equals(df)) {
                        result.append(",time");
                        break;
                    }
	 ...
	 * This is equivalent to <blockquote>
	 * <code>(new {@link AbstractMessageFormat#AbstractMessageFormat(String) MessageFormat}(pattern)).{@link #format(java.lang.Object[], java.lang.StringBuffer, java.text.FieldPosition) format}(arguments, new StringBuffer(), null).toString()</code>
	 * </blockquote>
	 *
	 * @param pattern
	 *        the pattern string
	 ...
	 *            if the pattern is invalid, or if an argument in the <code>arguments</code> array
	 *            is not of the type expected by the format element(s) that use it.
	 */
    public static String format(String pattern, Object ... arguments) {
        AbstractMessageFormat temp = new AbstractMessageFormat(pattern);
	...
	@Override
	public Object clone() {
        AbstractMessageFormat other = (AbstractMessageFormat) super.clone();
	...
	public boolean equals(Object obj) {
        ...
        if (obj == null || getClass() != obj.getClass())
            return false;
        AbstractMessageFormat other = (AbstractMessageFormat) obj;
    ...
    public static class Field extends Format.Field {
		...
		protected Object readResolve() throws InvalidObjectException {
            if (this.getClass() != AbstractMessageFormat.Field.class) {
            ...
            if (false) { // if (argRecursion == 3){
                ...
                if (obj == null) {
                    ...
                    if (subFormatter instanceof ChoiceFormat) {
                        ...
                        if (arg.indexOf('{') >= 0) {
                            subFormatter = new AbstractMessageFormat(arg, locale);
                            obj = arguments;
                            arg = null;
                        }
                    }
					...
					subFormatter = createDateTimeInstance(DateFormat.SHORT, DateFormat.SHORT);// fix
        ...
        if (segments[SEG_TYPE].length() != 0) {
            ...
            case TYPE_TIME:
                ...
                if (mod >= 0 && mod < DATE_TIME_MODIFIER_KEYWORDS.length) {
                    if (type == TYPE_DATE) {
							newFormat = createDateFormat(DATE_TIME_MODIFIERS[mod]);
                    } else {
							newFormat = createTimeFormat(DATE_TIME_MODIFIERS[mod]);
                    }
                } else {
                    // SimpleDateFormat pattern
                    try {
                        newFormat = createSimpleDateFormat(segments[SEG_MODIFIER]);
                    } catch (IllegalArgumentException e) {
                        maxOffset = oldMaxOffset;
                        throw e;
                    }
            ...
            default:
					newFormat = createCustomFormat(oldMaxOffset, segments[SEG_TYPE]);
            }
        }
	...
	protected Format createCustomFormat(int oldMaxOffset, String formatPattern) {
		maxOffset = oldMaxOffset;
		throw new IllegalArgumentException("unknown format type: " + formatPattern);
	}
	...
	/**
	 * Creates an AttributedCharacterIterator with the contents of <code>iterator</code> and the
	 * additional attribute <code>key</code> <code>value</code>.
	 *
	 * @param iterator
	...
	/**
	 * Returns an AttributedCharacterIterator with the String <code>string</code> and additional
	 * key/value pair <code>key</code>, <code>value</code>.
	 *
	 * @param string
	 *        String to create AttributedCharacterIterator from
	 * @param key
	...
	/**
	 * Creates an <code>AttributedCharacterIterator</code> containing the concatenated contents of
	 * the passed in <code>AttributedCharacterIterator</code>s.
	 *
	 * @param iterators
			...
			for (int counter = 0; counter < iterators.length; counter++) {
				...
				while (index < end) {
					...
					if (mapsDiffer(last, attrs)) {
						int endIndex = index - start + offset;
						as.addAttributes(attrs, beginIndex, endIndex);
						beginIndex = endIndex;
					}
					last = attrs;
