 ...
 */
public abstract class DBSchemaChangingCommand extends AbstractCommandHandler {
	...
	/**
	 * Creates a {@link DBSchemaChangingCommand} from configuration.
	 * 
	 * @param context
	 ...
	 */
	@CalledByReflection
	public DBSchemaChangingCommand(InstantiationContext context, Config config) {
	...
	// TODO JST Doku
	protected abstract HandlerResult changeSchema(Object model);

	// TODO JST Doku
	protected void writeTypes(String resource, List<MetaObjectName> types) throws XMLStreamException, IOException {
	...
	private static int kind(MetaObjectName t) {
	...
	private static <T extends ConfigurationItem> void writePretty(OutputStream out, String rootTag, Class<T> staticType,
		...
		try (OutputStreamWriter writer = new OutputStreamWriter(out, TypedConfiguration.DEFAULT_CHARSET)) {
