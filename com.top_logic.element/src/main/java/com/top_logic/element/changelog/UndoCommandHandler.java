...
/**
 * {@link CommandHandler} that undoes the newest real change in the change log window of its
 * target model.
 *
 * <p>
 * The target model is interpreted as a subtree root. A {@code null} model undoes the newest
...
public class UndoCommandHandler extends AbstractCommandHandler {
	...
	public interface Config extends AbstractCommandHandler.Config {
		...
		/**
		 * Whether to open a confirmation dialog when the undo cannot be applied cleanly.
		 */
		...
		/**
		 * Upper bound for the number of change log entries inspected to locate the change to
		 * undo. {@code 0} (default) means unlimited.
		 */
		@Name(MAX_ENTRIES)
		@IntDefault(0)
		 ...
		 * searching for the change to undo.
		 */
	...
	public HandlerResult handleCommand(DisplayContext context, LayoutComponent component, Object model,
		...
		ChangeSet target = ChangeSetReverter.findUndoCandidate(root, cfg.getMaxEntries(), cfg.getIncludeSubtree());
		if (target == null) {
