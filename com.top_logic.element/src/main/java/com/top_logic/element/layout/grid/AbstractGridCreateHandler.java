...
public abstract class AbstractGridCreateHandler extends PreconditionCommandHandler {
	...
	protected CommandStep prepare(LayoutComponent component, Object model, Map<String, Object> arguments) {
		...
		TLClass createType = getCreateType();
		boolean isAbstract = createType.isAbstract();
		if (isAbstract || openTypeChooser()) {
			Set<TLClass> concreteSpecializations = TLModelUtil.getConcreteSpecializations(createType);
			if (concreteSpecializations.isEmpty()) {
				String typeName = MetaLabelProvider.INSTANCE.getLabel(createType);
				ResKey errorKey = I18NConstants.ABSTRACT_TYPE_WITHOUT_SUBTYPES__TYPE.fill(typeName);
				return new Failure(errorKey);
			} else if (concreteSpecializations.size() == 1) {
				return successSingleOption(grid, model, concreteSpecializations.iterator().next());
			} else {
				return successMultipleOptions(grid, model, arguments, concreteSpecializations,
					isAbstract ? null : createType);
			}
		} else {
			return successSingleOption(grid, model, createType);
		}
	}
	...
	/**
	 * Whether to open a sub-type chooser, even if {@link #getCreateType()} is concrete.
	 */
	 ...
	 */
	protected abstract TLClass getCreateType();
	...
	private Success successMultipleOptions(GridComponent grid, Object model, Map<String, Object> arguments,
			Set<TLClass> concreteTypes, TLClass defaultOption) {
		...
		return new Success() {
			...
			private void openSelectDialog(DisplayContext aContext, HandlerResult suspended,
					Set<TLClass> concreteSpecializations) {
