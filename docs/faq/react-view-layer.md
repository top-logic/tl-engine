...
Such a page is linkable: `/view/tickets?filter=discussed` opens the list filtered by that preset, `?q=login` opens it searched for that text.
...
- **`search-term="ch"`** publishes the text the table searches for, and nothing while it searches for none.
...
- **The sharp edge**: searching for a text while a preset matches *ends* that match, because the term is one of the criteria a named filter is compared by — `active-preset` then holds nothing, and a query binding drops `filter` from the address as `q` enters it. The other way round, picking a preset replaces the whole filtering, the search term included.
