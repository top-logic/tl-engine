import { React as T, useTLState as j, useTLCommand as b, register as y } from "tl-react-bridge";
import { styleTags as Y, tags as O, LRParser as C, LRLanguage as Z, LanguageSupport as E, CodeEditor as x } from "tl-code-editor";
const R = Y({
  Number: O.number,
  String: O.string,
  Boolean: O.bool,
  ...
  '"?" ":"': O.punctuation
}), D = { __proto__: null, true: 40, false: 42, null: 46, tuple: 62, switch: 68, default: 74 }, W = C.deserialize({
  version: 14,
  ...
  tokenizers: [0, 1, 2, 3],
  topRules: { Script: [0, 3] },
  specialized: [{ term: 62, get: (i) => D[i] || -1 }],
  tokenPrec: 1011
}), $ = Z.define({
  parser: W.configure({
    props: [R]
...
const { useRef: q, useEffect: v, useCallback: m, useMemo: z } = T, G = 3e3, U = ({ controlId: i, state: S }) => {
  const p = j(), P = b(), n = q(null), f = S.value ?? "", V = S.readOnly === !0, _ = p.diagnostics ?? [], g = z(() => k(), []), h = m(
    (e) => {
      const t = e.pos, r = e.state.doc.lineAt(t);
      if (!e.matchBefore(/[\w$`.:]+/) && !e.explicit) return Promise.resolve(null);
      const a = e.matchBefore(/[\w]+/), u = (a == null ? void 0 : a.text) ?? "", Q = (a == null ? void 0 : a.from) ?? t, d = r.text.substring(0, t - r.from), o = String(Date.now()) + Math.random();
      return P("complete", { line: d, prefix: u, requestId: o }), new Promise((c) => {
        n.current = { requestId: o, resolve: c, from: Q }, setTimeout(() => {
          var X;
          ((X = n.current) == null ? void 0 : X.requestId) === o && (n.current = null, c(null));
        }, G);
      });
    },
    [P]
  ...
  v(() => {
    ...
    if (!e || !s || s.requestId !== e.requestId)
      return;
    const t = s.completions ?? [];
    if (n.current = null, t.length === 0) {
      e.resolve(null);
      return;
    }
    e.resolve({
      from: e.from,
      options: t.map((r) => {
        const l = r.replacement ?? r.name, a = r.cursorOffset, u = a == null ? l : (Q, d, o, c) => {
          Q.dispatch({
            changes: { from: o, to: c, insert: l },
            selection: { anchor: o + a }
          });
        };
        return {
          label: r.name,
  ...
  const w = m(
    (e) => {
      P("valueChanged", { value: e }), P("validate", { text: e });
    },
    [P]
  );
