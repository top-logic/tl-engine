 ...
 */
package com.top_logic.basic.shared.html;
...
public class TagUtilShared {
	...
	public static void writeAttribute(Appendable out, String attributeName, int value) {
		try {
			beginAttribute(out, attributeName);
			StringServicesShared.append(out, value);
			endAttribute(out);
		} catch (IOException ex) {
			throw new IllegalStateException(ex);
		}
	...
	/**
	 * Writes the given value within an XML attribute that was quoted using double quotes.
	 */
	public static void writeAttributeTextDQuot(Appendable out, CharSequence value) {
		if (value == null) {
			return;
		}
	 ...
	 *        the number of characters to encode.
	 */
	public static void writeAttributeTextDQuot(Writer out, char[] buffer, int offset, int length) {
		int closing = 0;
		for (int i = offset, cnt = offset + length; i < cnt; i++) {
			char ch = buffer[i];
			closing = writeAttributeTextDQuot(out, closing, ch);
	...
	private static int indexOfCDATAEnd(char[] content, int offset, int length, int fromIndex) {
		return StringServicesShared.indexOf(content, offset, length, CDATA_END_CHARS, 0, CDATA_END_CHARS.length,
			fromIndex);
	}
	...
	public static void writeCDataContent(Writer out, CharSequence content) throws IOException {
		...
		int pos = StringServicesShared.indexOf(content, CDATA_END);
		if (pos < 0) {
			// Save to write directly.
			out.append(content);
		} else {
			...
			do {
				...
				breakCDataSection(out);
				start = nextChunk;
				pos = StringServicesShared.indexOf(content, CDATA_END, start + 1);
			} while (pos >= 0);
	...
	/**
	 * Stops and re-starts a CDATA section.
	 */
	public static void breakCDataSection(Writer out) throws IOException {
	...
	/**
	 * Writes the given value as a quoted <i>JavaScript</i> string.
	 */
	public static void writeDirectJsString(Appendable out, CharSequence value) {
		if (value == null) {
	...
	/**
	 * Writes the given character as <i>JavaScript</i> string contents.
	 */
	public static void writeDirectJsStringContent(Appendable out, char value) {
		try {
			switch (value) {
				case 0x00:
				case 0x01:
	...
	/**
	 * Writes a JavaScript string quote character to the given appendable.
	 */
	protected static void writeDirectJsStringQuote(Appendable out) {
		try {
	...
	/**
	 * Appends a Unicode escape sequence for the given character to the given appendable.
	 */
	public static void encodeUnicodeJS(Appendable out, char value) throws IOException {
		out.append("\\u");
