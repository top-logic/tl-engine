package com.top_logic.react.flow.data;

/**
 * A connection in a {@link TreeLayout} that links contents of the diagram with one-to-many edges.
 */
public interface TreeConnection extends Widget, com.top_logic.react.flow.operations.tree.TreeConnectionOperations {

	/**
	 * Creates a {@link com.top_logic.react.flow.data.TreeConnection} instance.
	 */
	static com.top_logic.react.flow.data.TreeConnection create() {
		return new com.top_logic.react.flow.data.impl.TreeConnection_Impl();
	}

	/** Identifier for the {@link com.top_logic.react.flow.data.TreeConnection} type in JSON format. */
	String TREE_CONNECTION__TYPE = "TreeConnection";

	/** @see #getOwner() */
	String OWNER__PROP = "owner";

	/** @see #getParent() */
	String PARENT__PROP = "parent";

	/** @see #getChild() */
	String CHILD__PROP = "child";

	/** @see #getStrokeStyle() */
	String STROKE_STYLE__PROP = "strokeStyle";

	/** @see #getThickness() */
	String THICKNESS__PROP = "thickness";

	/** @see #getDashes() */
	String DASHES__PROP = "dashes";

	/** @see #getDecorations() */
	String DECORATIONS__PROP = "decorations";

	/** @see #getBarPosition() */
	String BAR_POSITION__PROP = "barPosition";

	/**
	 * The {@link TreeLayout} that is responsible for this connection.
	 */
	com.top_logic.react.flow.data.TreeLayout getOwner();

	/**
	 * Checks, whether {@link #getOwner()} has a value.
	 */
	boolean hasOwner();

	/**
	 * The connector that is anchored to the parent node of the connection.
	 */
	com.top_logic.react.flow.data.TreeConnector getParent();

	/**
	 * @see #getParent()
	 */
	com.top_logic.react.flow.data.TreeConnection setParent(com.top_logic.react.flow.data.TreeConnector value);

	/**
	 * Checks, whether {@link #getParent()} has a value.
	 */
	boolean hasParent();

	/**
	 * The connector that is  anchored to the child node of the connection.
	 */
	com.top_logic.react.flow.data.TreeConnector getChild();

	/**
	 * @see #getChild()
	 */
	com.top_logic.react.flow.data.TreeConnection setChild(com.top_logic.react.flow.data.TreeConnector value);

	/**
	 * Checks, whether {@link #getChild()} has a value.
	 */
	boolean hasChild();

	/**
	 * The SVG <code>stroke</code> style. If not set, the default from the {@link TreeLayout} is used.
	 */
	String getStrokeStyle();

	/**
	 * @see #getStrokeStyle()
	 */
	com.top_logic.react.flow.data.TreeConnection setStrokeStyle(String value);

	/**
	 * Checks, whether {@link #getStrokeStyle()} has a value.
	 */
	boolean hasStrokeStyle();

	/**
	 * The thickness of the drawn border. If not set, the default from the {@link TreeLayout} is used.
	 */
	Double getThickness();

	/**
	 * @see #getThickness()
	 */
	com.top_logic.react.flow.data.TreeConnection setThickness(Double value);

	/**
	 * Checks, whether {@link #getThickness()} has a value.
	 */
	boolean hasThickness();

	/**
	 * Length of dashes for a dashed border.
	 */
	java.util.List<Double> getDashes();

	/**
	 * @see #getDashes()
	 */
	com.top_logic.react.flow.data.TreeConnection setDashes(java.util.List<? extends Double> value);

	/**
	 * Adds a value to the {@link #getDashes()} list.
	 */
	com.top_logic.react.flow.data.TreeConnection addDash(double value);

	/**
	 * Removes a value from the {@link #getDashes()} list.
	 */
	void removeDash(double value);

	/**
	 * Decorations to place on the connection.
	 */
	java.util.List<com.top_logic.react.flow.data.EdgeDecoration> getDecorations();

	/**
	 * @see #getDecorations()
	 */
	com.top_logic.react.flow.data.TreeConnection setDecorations(java.util.List<? extends com.top_logic.react.flow.data.EdgeDecoration> value);

	/**
	 * Adds a value to the {@link #getDecorations()} list.
	 */
	com.top_logic.react.flow.data.TreeConnection addDecoration(com.top_logic.react.flow.data.EdgeDecoration value);

	/**
	 * Removes a value from the {@link #getDecorations()} list.
	 */
	void removeDecoration(com.top_logic.react.flow.data.EdgeDecoration value);

	/**
	 * The placement of the connection bar that represents the fan-out from the one parent to its many children. 
	 * The bar is rendered perpendicular to the line connecting the nodes with the bar. 
	 * The position of the bar is given as absolute coordinate (X or Y depending on the layout direction) within 
	 * the coordinate system of the {@link TreeLayout}.
	 */
	double getBarPosition();

	/**
	 * @see #getBarPosition()
	 */
	com.top_logic.react.flow.data.TreeConnection setBarPosition(double value);

	@Override
	com.top_logic.react.flow.data.TreeConnection setCssClass(String value);

	@Override
	com.top_logic.react.flow.data.TreeConnection setUserObject(java.lang.Object value);

	@Override
	com.top_logic.react.flow.data.TreeConnection setClientId(String value);

	@Override
	com.top_logic.react.flow.data.TreeConnection setRenderInfo(java.lang.Object value);

	/** Reads a new instance from the given reader. */
	static com.top_logic.react.flow.data.TreeConnection readTreeConnection(de.haumacher.msgbuf.graph.Scope scope, de.haumacher.msgbuf.json.JsonReader in) throws java.io.IOException {
		if (in.peek() == de.haumacher.msgbuf.json.JsonToken.NUMBER) {
			return (com.top_logic.react.flow.data.TreeConnection) scope.resolveOrFail(in.nextInt());
		}
		in.beginArray();
		String type = in.nextString();
		assert TREE_CONNECTION__TYPE.equals(type);
		int id = in.nextInt();
		com.top_logic.react.flow.data.impl.TreeConnection_Impl result = new com.top_logic.react.flow.data.impl.TreeConnection_Impl();
		scope.readData(result, id, in);
		in.endArray();
		return result;
	}

	@Override
	default TreeConnection self() {
		return this;
	}

}
