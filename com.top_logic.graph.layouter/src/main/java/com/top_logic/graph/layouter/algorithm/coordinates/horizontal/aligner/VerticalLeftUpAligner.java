...
public class VerticalLeftUpAligner extends VerticalUpAligner {
	...
	/**
	 * Creates a {@link VerticalLeftUpAligner}.
	 */
	public VerticalLeftUpAligner(LayoutGraph graph, LayoutDirection direction) {
		...
		for (int i = amountOfLayers - 1; i >= 1; i--) {
			...
			for (int k = 0; k < nodesSize; k++) {
				...
				if (!currentNode.isSourceDummy()) {
					...
					List<Integer> medianIndices = getMedianIndices(lowerNodes, false);

					for (int medianIndex : medianIndices) {
						if (getAligns().get(currentNode) == currentNode) {
							LayoutNode median = lowerNodes.get(medianIndex);

							Collection<LayoutEdge> edges = LayoutGraphUtil.getEdges(getDirection(), getGraph(), currentNode, median);
